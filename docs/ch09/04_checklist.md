...
- [ ] 场景 4（会话存档 + 工具记录）：启动 → 输入 "读取 pyproject.toml" → 模型调 read_file → 回复内容；`cat .mewcode/sessions/*/conversation.jsonl` 至少 4 行（user → assistant+tool_calls → tool_results → assistant）；`grep tool_calls` 和 `grep tool_results` 各至少一条命中。(AC8/AC9)
