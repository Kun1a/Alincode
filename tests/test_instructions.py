...
def test_single_layer(tmp_path):
    """只有项目根 ALINCODE.md 有内容。"""
    (tmp_path / "ALINCODE.md").write_text("project rules", encoding="utf-8")
    loader = Loader(project_root=str(tmp_path), user_home=str(tmp_path))
    result = loader.load()
...
def test_include_expand(tmp_path):
    """@include 正常展开。"""
    (tmp_path / "ALINCODE.md").write_text("main\n@include rules.md\nend", encoding="utf-8")
...
def test_include_depth_limit(tmp_path):
    ...
    (tmp_path / "ALINCODE.md").write_text("@include a.md", encoding="utf-8")
    loader = Loader(project_root=str(tmp_path), user_home=str(tmp_path))
    result = loader.load()
...
def test_include_escape(tmp_path):
    """路径逃逸 → 不加载。"""
    (tmp_path / "ALINCODE.md").write_text("@include ../outside.md", encoding="utf-8")
    loader = Loader(project_root=str(tmp_path), user_home=str(tmp_path))
    result = loader.load()
...
def test_legacy_mewcode_file_is_not_loaded(tmp_path):
    (tmp_path / "MEWCODE.md").write_text("legacy rules", encoding="utf-8")
    loader = Loader(project_root=str(tmp_path), user_home=str(tmp_path))
    assert loader.load() == ""
